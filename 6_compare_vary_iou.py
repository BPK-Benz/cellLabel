...
if __name__ == "__main__":
	...
	for threshold_iou in list(np.arange(0, 1, 0.05)):
		...
		for index in range(total):
			...
			for c in categories:
				...
				scores[c['name']]['tp'] += len(tps)
				scores[c['name']]['fp'] += len(fps) + len(matched_gt) - len(tps)
				scores[c['name']]['fn'] += len(fns)
		...
		for c in categories:
			...
			row[c['name'] + '_precision'] = precision
			row[c['name'] + '_recall'] = recall
			row[c['name'] + '_f1'] = f1

		print('total_instances', total_instances)
		print('checksum_scores', checksum(scores))

		row['iou'] = threshold_iou
		df = df.append(row, ignore_index=True)

	df_path = 'eval_vary_iou.csv'
	df.to_csv(df_path)  